...
// ResolveCode reports the activity and QR code id a scanned code
// belongs to. Purpose-built for the attendance domain's QRCodeLookup
// interface — same reasoning as HangoutRepository.Attended for
// rating's AttendanceChecker — so that package never needs to
// import this package's domain types.
func (r *QRCodeRepository) ResolveCode(ctx context.Context, code string) (activityID, qrCodeID uuid.UUID, found bool, err error) {
	err = r.pool.QueryRow(ctx,
		`SELECT activity_id, id FROM qr_codes WHERE code = $1`, code,
	).Scan(&activityID, &qrCodeID)
	if err != nil {
	...
	return activityID, qrCodeID, true, nil
}
